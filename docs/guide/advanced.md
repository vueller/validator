...
const hasProfileErrors = Object.keys(profileErrors)
  .some(key => key.startsWith('userProfileForm.'));
```
...
validator.extend('divisibleBy', (value, divisor) => {
...
validator.extend('requiredIf', function(value, targetField, targetValue) {
...
validator.extend('requiredIfChecked', function(value, checkboxField) {
...
validator.extend('validCreditCard', function(value, cardTypeField) {
  if (!value) return true;
...
validator.extend('arrayLength', (value, min, max) => {
